...
def projector_factor_fn(ckpt,fact,files):
    ...
    filename = os.path.splitext(os.path.basename(files[0]))[0] + ".pt" # args.files[0]
...
if __name__ == "__main__":
    ...
    for i, input_name in enumerate(args.files):
        ...
        img_name = os.path.splitext(os.path.basename(input_name))[0] + "-project.png"
        pil_img = Image.fromarray(img_ar[i])
        pil_img.save(factor_base_path+img_name) # add factor_base_path

    torch.save(result_file, factor_base_path+filename) # factor_base_path
